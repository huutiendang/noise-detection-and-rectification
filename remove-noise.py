...
def rank_by_similarity(df_noise, df_noise_name, df_clean, n_sample, dir_checkpoint, save_path, feature_method, k):
    n_noise = df_noise["isFlipped"].sum()
    ...
    ranked_indices = np.argsort(scores)[:n_noise]
    df_noise_removed = df_noise.drop(ranked_indices).reset_index(drop=True) 
    df_noise_removed.to_csv(os.path.join(save_path, f"{feature_method}-removed-{os.path.basename(df_noise_name)}"), sep="\t", index=False)
    #return df_noise.drop(ranked_indices).reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    cos = rank_by_similarity(df_clean=df_cle, df_noise=df_noi, feature_method='cos', k=opt.k, dir_checkpoint=opt.dir_checkpoint, n_sample=opt.n_sample, save_path=opt.save_path, df_noise_name=opt.df_noise)
    dot = rank_by_similarity(df_clean=df_cle, df_noise=df_noi, feature_method='dot', k=opt.k, dir_checkpoint=opt.dir_checkpoint, n_sample=opt.n_sample, save_path=opt.save_path, df_noise_name=opt.df_noise)
