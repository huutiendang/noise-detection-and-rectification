...
def similarity_detection(df_noise, df_clean, n_sample, feature_method, k):
    dis = pd.DataFrame(np.load(feature_method))
...
def arguments():
    ...
    parser.add_argument('--similarity', choices=['dot', 'cos', 'euc'], required=True, help='similarity')
    parser.add_argument('--seed', type=int, required=True, help='random seed')
    parser.add_argument('--n_sample', type=int, required=True, help='the number of sampling')
    parser.add_argument('--k', type=int, required=True, help='k nearest neighbors')
...
if __name__ == "__main__":
    ...
    print(f"Checkpoint: {opt.dir_checkpoint}, k: {opt.k}, len of the aux: {opt.n_sample}")
    feature_method = os.path.join(opt.dir_checkpoint, opt.similarity) + '.npy'
    result = {f"{opt.similarity}": similarity_detection(df_noise, df_clean, opt.n_sample, feature_method, opt.k)}

    
    with open(os.path.join(opt.dir_checkpoint, f'{opt.similarity}_n_sample_{opt.n_sample}_k_{opt.k}.json'), 'w') as f:
        json.dump(result, f)
